...
func resourceGlesysDomain() *schema.Resource {
	return &schema.Resource{
		Create: resourceGlesysDomainCreate,
		Read:   resourceGlesysDomainRead,
		Update: resourceGlesysDomainUpdate,
		Delete: resourceGlesysDomainDelete,
...
func resourceGlesysDomainCreate(d *schema.ResourceData, m interface{}) error {
	client := m.(*glesys.Client)

	// Add a domain in the glesys platform. Do not register new domains right now.
	params := glesys.AddDomainParams{
		Name:              d.Get("name").(string),
		CreateRecords:     d.Get("createrecords").(bool),
	...
	domain, err := client.Domains.AddDomain(context.Background(), params)
	if err != nil {
	...
	return resourceGlesysDomainRead(d, m)
}

func resourceGlesysDomainRead(d *schema.ResourceData, m interface{}) error {
	client := m.(*glesys.Client)

	domain, err := client.Domains.Details(context.Background(), d.Id())
...
func resourceGlesysDomainUpdate(d *schema.ResourceData, m interface{}) error {
	client := m.(*glesys.Client)

	params := glesys.EditDomainParams{Name: d.Id()}
	...
	_, err := client.Domains.Edit(context.Background(), params)
	if err != nil {
...
func resourceGlesysDomainDelete(d *schema.ResourceData, m interface{}) error {
	client := m.(*glesys.Client)

	params := glesys.DeleteDomainParams{
		Name: d.Id(),
	}

	err := client.Domains.Delete(context.Background(), params)
	if err != nil {
